...
client.on('message', (name, tags, message, self) => {
	...
	showPopup();
		usersElement.textContent = Object.keys(users);
			...
			if (listeningForPlay == true && command === "stand") { // enough
			if (Object.keys(users) == username) {
				users[tags.username] = true;
			blackjackStand();
			}}
			if (listeningForPlay == true && command === "deal") { // end round
			if (Object.keys(users) == username) {
				users[tags.username] = true;
			blackjackDeal();
...
/* only for testing if the bot can write into the chat */
		if(command === 'echo') {
...
// clean the overlay when command /clear is used
